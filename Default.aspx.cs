                ...
                if (int.TryParse(e.CommandArgument.ToString(), out docId))
                {
                    ...
                    try
                    {
                        using (SqlConnection con = new SqlConnection(connectionString))
                        {
                            ...
                            using (SqlCommand cmdVersions = new SqlCommand("DELETE FROM DocumentVersions WHERE DocumentID = @DocumentID", con))
                            {
                                cmdVersions.Parameters.AddWithValue("@DocumentID", docId);
                                cmdVersions.ExecuteNonQuery();
                            }
                            
                            using (SqlCommand cmdCollab = new SqlCommand("DELETE FROM Collaborators WHERE DocumentID = @DocumentID", con))
                            {
                                cmdCollab.Parameters.AddWithValue("@DocumentID", docId);
                                cmdCollab.ExecuteNonQuery();
                            }
                            
                            using (SqlCommand cmdFiles = new SqlCommand("DELETE FROM Files WHERE DocumentID = @DocumentID", con))
                            {
                                cmdFiles.Parameters.AddWithValue("@DocumentID", docId);
                                cmdFiles.ExecuteNonQuery();
                            }

                            // Delete comments
                            using (SqlCommand cmdComments = new SqlCommand("DELETE FROM Comments WHERE DocumentID = @DocumentID", con))
                            {
                                cmdComments.Parameters.AddWithValue("@DocumentID", docId);
